...
checkBtn.addEventListener('click', ()=>{
    ...
    if(!Number.isInteger(cashRecievedValue)){
      showError("Please enter valid amount in cash recieved field");
      return;
    }
    if(billAmtValue > cashRecievedValue){
      showError("Oops! Seems like you have to give more cash");
      return;
    }
  
    calculateNotes(billAmtValue, cashRecievedValue);
  } else {
      showError("Please enter valid amount in both the fields to continue");
    }
})
...
function calculateNotes(bill, cash){
    ...
    if(returnAmt<1){
      showError("No amount needs to be returned");
      return;
    }
